...
if (!is_object($xoopsUser)) {
    $errormessage = _MD_PM_SORRY . '<br>' . _MD_PM_PLZREG . '';
    ...
    for ($i = 0; $i < $total_messages; ++$i) {
        ...
        if ($pm_arr[$i]->getVar('read_msg') == 1) {
            echo "<td class='aligntop width5 txtcenter'><img src='images/email_read.png' alt='" . _MD_PM_READ . "' title='" . _MD_PM_READ . "' /></td>\n";
        } else {
            echo "<td class='aligntop width5 txtcenter'><img src='images/email_notread.png' alt='" . _MD_PM_NOTREAD . "' title='" . _MD_PM_NOTREAD . "' /></td>\n";
        }
    ...
    if ($display == 1) {
        echo "<tr class='foot txtleft'><td colspan='6' align='left'><input type='button' class='formButton' onclick='openWithSelfMain(\"" . XOOPS_URL . "/pmlite.php?send=1\",\"pmlite\",565,500);' value='" . _MD_PM_SEND . "' />&nbsp;<input type='submit' class='formButton' name='delete_messages' value='" . _MD_PM_DELETE . "' />" . $GLOBALS['xoopsSecurity']->getTokenHTML() . '</td></tr></table></form>';
    } else {
        echo "<tr class='bg2 txtleft'><td class='txtleft' colspan='6'><input type='button' class='formButton' onclick='openWithSelfMain(\"" . XOOPS_URL . "/pmlite.php?send=1\",\"pmlite\",565,500);' value='" . _MD_PM_SEND . "' /></td></tr></table></form>";
    }
    include $GLOBALS['xoops']->path('footer.php');
}
