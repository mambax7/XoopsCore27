...
if (!$tpl->isCached('db:system_rss.tpl')) {
    ...
    $tpl->assign('channel_title', XoopsLocal::convert_encoding(htmlspecialchars((string) $xoopsConfig['sitename'], ENT_QUOTES | ENT_HTML5)));
    $tpl->assign('channel_link', XOOPS_URL . '/');
    $tpl->assign('channel_desc', XoopsLocal::convert_encoding(htmlspecialchars((string) $xoopsConfig['slogan'], ENT_QUOTES | ENT_HTML5)));
    ...
    if (!empty($sarray) && \is_array($sarray)) {
        foreach ($sarray as $story) {
            $tpl->append('items', [
                'title'       => XoopsLocal::convert_encoding(htmlspecialchars((string) $story->title(), ENT_QUOTES | ENT_HTML5)),
                'link'        => XOOPS_URL . '/modules/news/article.php?storyid=' . $story->storyid(),
                'guid'        => XOOPS_URL . '/modules/news/article.php?storyid=' . $story->storyid(),
                'pubdate'     => formatTimestamp($story->published(), 'rss'),
                'description' => XoopsLocal::convert_encoding(htmlspecialchars((string) $story->hometext(), ENT_QUOTES | ENT_HTML5))
            ]);
        }
    }
}
