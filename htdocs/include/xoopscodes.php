...
function xoopsSmilies($textarea_id)
{
    ...
    $count  = count($smiles);
    for ($i = 0; $i < $count; ++$i) {
        echo "<img src='" . XOOPS_UPLOAD_URL . '/' . htmlspecialchars((string) $smiles[$i]['smile_url'], ENT_QUOTES | ENT_HTML5) . "' border='0' alt='' onclick='xoopsCodeSmilie(\"{$textarea_id}\", \" " . $smiles[$i]['code'] . " \");' onmouseover='style.cursor=\"hand\"' />";
    }
