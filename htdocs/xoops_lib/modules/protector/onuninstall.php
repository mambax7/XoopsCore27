...
if (!function_exists('protector_onuninstall_base')) {
    ...
    function protector_onuninstall_base($module, $mydirname)
    {
        ...
        if (file_exists($sql_file_path)) {
            $ret[]     = 'SQL file found at <b>' . htmlspecialchars((string) $sql_file_path, ENT_QUOTES | ENT_HTML5) . '</b>.<br  /> Deleting tables...<br>';
            $sql_lines = file($sql_file_path);
            foreach ($sql_lines as $sql_line) {
                if (preg_match('/^CREATE TABLE \`?([a-zA-Z0-9_-]+)\`? /i', $sql_line, $regs)) {
                    $sql = 'DROP TABLE ' . addslashes($prefix_mod . '_' . $regs[1]);
                    if (!$db->query($sql)) {
                        $ret[] = '<span style="color:#ff0000;">ERROR: Could not drop table <b>' . htmlspecialchars((string) $prefix_mod . '_' . $regs[1], ENT_QUOTES | ENT_HTML5) . '<b>.</span><br>';
                    } else {
                        $ret[] = 'Table <b>' . htmlspecialchars((string) $prefix_mod . '_' . $regs[1], ENT_QUOTES | ENT_HTML5) . '</b> dropped.<br>';
                    }
                }
            }
        }
