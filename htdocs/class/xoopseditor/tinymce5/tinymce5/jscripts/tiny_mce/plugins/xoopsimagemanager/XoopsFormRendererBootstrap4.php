...
class XoopsFormRendererBootstrap4 implements XoopsFormRendererInterface
{
    ...
    public function renderFormFile(XoopsFormFile $element)
    {
        return '<input type="hidden" name="MAX_FILE_SIZE" value="' . $element->getMaxFileSize() . '">'
            . '<input type="file" class="form-control-file"  name="' . $element->getName()
            . '" id="' . $element->getName()
    ...
    public function renderThemeForm(XoopsThemeForm $form)
    {
        ...
        foreach ($form->getElements() as $element) {
            ...
            if (($caption = $element->getCaption()) != '') {
                $ret .= '<label for="' . $element->getName() . '" class="col-3 col-form-label text-sm-right">'
                    . $element->getCaption()
                    . ($element->isRequired() ? '<span class="caption-required text-danger">*</span>' : '')
                    . '</label>';
            } else {
                $ret .= '<div class="col-3"> </div>';
            }
            $ret .= '<div class="col-9">';
            $ret .= $element->render();
