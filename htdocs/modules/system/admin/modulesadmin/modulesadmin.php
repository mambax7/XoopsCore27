            ...
            if (!$module_handler->insert($module)) {
                ...
                if ($blocks !== false) {
                    ...
                    foreach ($blocks as $blockkey => $block) {
                        ...
                        $sql        = 'INSERT INTO ' . $db->prefix('newblocks') . " (bid, mid, func_num, options, name, title, content, side, weight, visible, block_type, c_type, isactive, dirname, func_file, show_func, edit_func, template, bcachetime, last_modified) VALUES ($newbid, $newmid, " . (int)$blockkey . ", '" . addslashes($options) . "', '" . $block_name . "','" . $block_name . "', '', 0, 0, 0, 'M', 'H', 1, '" . addslashes($dirname) . "', '" . addslashes(trim((string) $block['file'])) . "', '" . addslashes(trim((string) $block['show_func'])) . "', '" . addslashes($edit_func) . "', '" . addslashes($template) . "', 0, " . time() . ')';
                        if (!$db->exec($sql)) {
