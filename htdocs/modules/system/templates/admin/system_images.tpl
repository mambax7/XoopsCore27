        ...
        <div class="xo-buttons">
            <{if empty($edit_form) && empty($listimg) && empty($multiupload)}>
                <button id="xo-addcat-btn" class="ui-corner-all tooltip" onclick="xo_toggle('div#xo-category-add');"
                    ...
                    <img src="<{xoAdminIcons url='add.png'}>" alt="<{$smarty.const._AM_SYSTEM_IMAGES_ADDCAT}>"/>
            ...
            <{if !empty($cat_img) || !empty($listimg)}>
                <button id="xo-addimg-btn" class="ui-corner-all tooltip" onclick="xo_toggle('div#xo-images-add');"
                    ...
                    <img src="<{xoAdminIcons url='add.png'}>" alt="<{$smarty.const._AM_SYSTEM_IMAGES_ADDIMG}>"/>
            ...
            <{if !empty($listimg)}>
                <button id="xo-addavatar-btn" class="ui-corner-all tooltip" onclick='location="admin.php?fct=images&amp;op=multiupload&amp;imgcat_id=<{$imgcat_id}>"'
                    ...
                    <img src="<{xoAdminIcons url='add.png'}>" alt="<{$smarty.const._AM_SYSTEM_IMAGES_MULTIUPLOAD}>"/>
            ...
            <tr class="<{cycle values='odd, even'}> txtcenter">
                ...
                <td class="xo-actions"><img id="loading_cat<{$cat.id}>" src="./images/spinner.gif" style="display:none;"
                                            alt="<{$smarty.const._AM_SYSTEM_LOADING}>"/><img class="cursorpointer tooltip" id="cat<{$cat.id}>"
                                                                                             ...
                                                                                             src="<{if $cat.display}><{xoAdminIcons url='success.png'}><{else}><{xoAdminIcons url='cancel.png'}><{/if}>"
                                                                                             alt=""
                                                                                             title="<{if $cat.display}><{$smarty.const._AM_SYSTEM_IMAGES_OFF}><{else}><{$smarty.const._AM_SYSTEM_IMAGES_ON}><{/if}>"/>
                </td>
                <td class="xo-actions txtcenter">
                    <a class="tooltip" href="admin.php?fct=images&amp;op=listimg&amp;imgcat_id=<{$cat.id}>" title="<{$smarty.const._AM_SYSTEM_IMAGES_VIEW}>">
                        <img src="<{xoAdminIcons url='display.png'}>" alt="<{$smarty.const._AM_SYSTEM_IMAGES_VIEW}>"/></a>
                    <{if isset($xoops_isadmin)}>
                        ...
                        <a class="tooltip" href="admin.php?fct=images&amp;op=delcat&amp;imgcat_id=<{$cat.id}>" title="<{$smarty.const._DELETE}>">
                            <img src="<{xoAdminIcons url='delete.png'}>" alt=""/></a>
                    <{/if}>
                </td>
            </tr>
        <{/foreach}>
                ...
                <div class="ui-corner-all xo-thumb txtcenter">
                    ...
                    <div class="xo-actions txtcenter">
						...
						<img class="cursorpointer tooltip" id="img<{$img.image_id}>"
							...
							src="<{if $img.image_display}><{xoAdminIcons url='success.png'}><{else}><{xoAdminIcons url='cancel.png'}><{/if}>"
						...
						<{/if}>
						<img src="<{xoAdminIcons url='display.png'}>" alt="<{$smarty.const._AM_SYSTEM_IMAGES_VIEW}>"/></a>
						<a class="tooltip" href="admin.php?fct=images&amp;op=editimg&amp;image_id=<{$img.image_id}>" title="<{$smarty.const._EDIT}>"><img src="<{xoAdminIcons url='edit.png'}>" alt="<{$smarty.const._EDIT}>"/></a>
                        <a class="tooltip" href="admin.php?fct=images&amp;op=delfile&amp;image_id=<{$img.image_id}>" title="<{$smarty.const._DELETE}>"><img src="<{xoAdminIcons url='delete.png'}>" alt="<{$smarty.const._DELETE}>"/></a>
						<img class="tooltip" onclick="display_dialog(<{$img.image_id}>, true, true, 'slide', 'slide', 120, 350);" src="<{xoAdminIcons url='url.png'}>" alt="<{$smarty.const._AM_SYSTEM_IMAGES_URL}>" title="<{$smarty.const._AM_SYSTEM_IMAGES_URL}>"/>
                    </div>
                </div>
            </div>
