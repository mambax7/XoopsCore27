...
final class Challenge2faTemplateTest extends TestCase
{
    ...
    public function testChallengeTextIsEscapedAndErrorsKeepTheirLineBreaks(): void
    {
        ...
        try {
            ...
            self::assertStringNotContainsString('Send again', $html);
            $smarty->assign(['standalone' => false, 'by_email' => true]);
            ...
            self::assertStringContainsString('name="xoops_2fa_send"', $themed);
            self::assertSame(2, substr_count($themed, 'name="XOOPS_TOKEN"'), 'the resend form carries its own token');
            $smarty->assign(['standalone' => true, 'by_email' => false]);
