...
The PR template's checklist links here for the background behind its "see notes" items.
...
- **Process-global state in tests** - save and restore everything a test touches: `$_SERVER`, ini settings, session or cookie state, `$GLOBALS['xoopsConfig']`. A defined constant cannot be unset or restored, so a constant-driven test is isolated in its own process with PHPUnit's `RunInSeparateProcess` **plus** `PreserveGlobalState(false)` - without the second attribute PHPUnit carries the parent's global state into the child process and the bootstrap-defined constant comes with it. The repository's existing pattern: `tests/unit/htdocs/class/xoopseditor/tinymce7/XoopsFormTinymce7Test.php`.
